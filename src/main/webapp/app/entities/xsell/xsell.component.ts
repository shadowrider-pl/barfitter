...
export class XsellComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.xsellService
      ...
      .subscribe((res: HttpResponse<IXsell[]>) => this.paginateXsells(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IXsell) {
    return item.id;
  }
  ...
  protected paginateXsells(data: IXsell[], headers: HttpHeaders) {
