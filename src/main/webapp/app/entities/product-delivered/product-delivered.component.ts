...
export class ProductDeliveredComponent implements OnInit, OnDestroy {
  currentAccount: any;
  productDelivereds: IProductDelivered[];
  error: any;
  success: any;
  ...
  loadAll() {
    this.productDeliveredService
      ...
      .subscribe((res: HttpResponse<IProductDelivered[]>) => this.paginateProductDelivereds(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IProductDelivered) {
    return item.id;
  }
  ...
  protected paginateProductDelivereds(data: IProductDelivered[], headers: HttpHeaders) {
