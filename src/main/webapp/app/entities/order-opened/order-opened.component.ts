...
@Component({
  ...
  templateUrl: './order-opened.component.html'
})
export class OrderOpenedComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.orderOpenedService
      ...
      .subscribe((res: HttpResponse<IOrderOpened[]>) => this.paginateOrderOpeneds(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/order-opened'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IOrderOpened) {
    return item.id;
  }
  ...
  protected paginateOrderOpeneds(data: IOrderOpened[], headers: HttpHeaders) {
