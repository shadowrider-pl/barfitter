...
export class ProductOrderedComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.productOrderedService
      .query({
        page: this.page - 1,
        size: this.itemsPerPage,
        sort: this.sort()
      })
      .subscribe((res: HttpResponse<IProductOrdered[]>) => this.paginateProductOrdereds(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/product-ordered'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IProductOrdered) {
    return item.id;
  }

  registerChangeInProductOrdereds() {
    this.eventSubscriber = this.eventManager.subscribe('productOrderedListModification', response => this.loadAll());
  }
  ...
  protected paginateProductOrdereds(data: IProductOrdered[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.productOrdereds = data;
  }
}
