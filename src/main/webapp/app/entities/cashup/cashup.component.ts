...
export class CashupComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.cashupService
      ...
      .subscribe((res: HttpResponse<ICashup[]>) => this.paginateCashups(res.body, res.headers));
  }
  ...
  trackId(index: number, item: ICashup) {
    return item.id;
  }

  registerChangeInCashups() {
    this.eventSubscriber = this.eventManager.subscribe('cashupListModification', response => this.loadAll());
  }
  ...
  protected paginateCashups(data: ICashup[], headers: HttpHeaders) {
