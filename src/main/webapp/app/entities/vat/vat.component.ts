...
export class VatComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.vatService
      ...
      .subscribe((res: HttpResponse<IVat[]>) => this.paginateVats(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IVat) {
    return item.id;
  }
  ...
  protected paginateVats(data: IVat[], headers: HttpHeaders) {
