...
export class PaymentDetailComponent implements OnInit, OnDestroy {
  ...
  constructor(private eventManager: JhiEventManager, private paymentService: PaymentService, private route: ActivatedRoute) {}
  ...
  load(id) {
    this.paymentService.find(id).subscribe((paymentResponse: HttpResponse<Payment>) => {
